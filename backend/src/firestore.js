...
let initState = { initialized: false, available: false, db: null, reason: '' };
...
function ensureFirestore() {
  if (initState.initialized) return initState;

  initState.initialized = true;
  const serviceAccount = readServiceAccount();
  if (!serviceAccount) {
    initState.reason = 'missing-service-account';
    return initState;
  }
  ...
  try {
    ...
    initState.db = admin.firestore();
    initState.available = true;
    return initState;
  } catch (e) {
    initState.available = false;
    initState.reason = e?.message || 'firebase-init-failed';
    return initState;
  }
}

export function isFirestoreConfigured() {
  return ensureFirestore().available;
}
...
function mapDocToItem(doc) {
  ...
  return {
    ...
    delivery: String(d.delivery || d.deliveryText || '2-4 days'),
...
export async function searchFirestoreProducts(query, limit = 40) {
  const state = ensureFirestore();
...
export async function suggestFirestore(query, limit = 8) {
  const state = ensureFirestore();
  if (!state.available) return [];
